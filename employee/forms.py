...
class EmployeeForm(forms.ModelForm):
    ...
    position = forms.ChoiceField(
        choices=[('', 'Select Position')] + list(Employee.POSITION_CHOICES),
        required=True,
        widget=forms.Select(attrs={'class': 'form-select'})
    )

    # Face image field
    face_image = forms.ImageField(required=False, widget=forms.FileInput(attrs={'class': 'form-control', 'accept': 'image/*'}))
